...
2. From the repo root, run `pdm install -G dev`.
3. From `frontend/`, run `npm ci`.
4. Optionally copy `.env.example` to `.env` for local overrides.
5. Run the dashboard once, then run the smoke checks.
...
2. Confirm BRFSS/EPA/integrated/DuckDB artifacts show as `Ready`
